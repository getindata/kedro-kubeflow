...
class PipelineGenerator(object):
    ...
    def _create_data_volume_init_op(self, kfp_ops: Dict[str, dsl.ContainerOp],
                                    image: str):
    ...
    def _generate_inputs(self, node: Node, node_dependencies: Dict[Node, Set[Node]]):
    ...
    def _build_kfp_ops(
        ...
        if is_mlflow_enabled():
            kfp_ops["mlflow-start-run"] = self._create_mlflow_op(image, tracking_token)
        ...
        for node in node_dependencies:
            ...
            output_specs, output_copy_commands, output_placeholders = self._generate_outputs(
                node)
            input_params, input_specs = self._generate_inputs(node, node_dependencies)
            mlflow_inputs, mlflow_tokens = self._generate_mlflow_inputs()
            component_params = [tracking_token, kfp_ops["mlflow-start-run"].output] \
                if is_mlflow_enabled() else []
            ...
            kfp_ops[name] = self._create_kedro_op(name, spec,
